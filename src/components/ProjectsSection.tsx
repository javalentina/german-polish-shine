    ...
    <section className="relative py-32 md:py-40 overflow-hidden">
      ...
      <div ref={ref} className="relative mx-auto max-w-5xl px-6">
        <div className={`mb-16 ${isVisible ? "animate-reveal-up" : "opacity-0"}`}>
          <p className="font-body text-xs tracking-[0.3em] uppercase text-primary mb-4">{t("projects.label")}</p>
        ...
        <div className="grid gap-px bg-border sm:grid-cols-2">
          {projects.map((project, i) => (
            <div key={i} className={`bg-background p-8 md:p-10 transition-colors hover:bg-secondary/30 ${isVisible ? "animate-reveal-up" : "opacity-0"}`}
              ...
              <p className="font-body text-sm leading-relaxed text-muted-foreground text-pretty">{project.description}</p>
            </div>
          ))}
        </div>

        <div className={`mt-16 border border-border p-8 md:p-12 ${isVisible ? "animate-reveal-up delay-600" : "opacity-0"}`}>
          <p className="font-body text-xs tracking-[0.3em] uppercase text-primary mb-4">{t("projects.podcast")}</p>
          <h3 className="font-display text-3xl font-light mb-4">{t("projects.podcast.title")}</h3>
          <p className="font-body text-sm text-muted-foreground mb-6 max-w-lg text-pretty">{t("projects.podcast.desc")}</p>
