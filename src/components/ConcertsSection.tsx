    ...
    <section id="concerts" className="py-32 md:py-40">
      <div ref={ref} className="mx-auto max-w-4xl px-6">
        <div className={`mb-16 text-center ${isVisible ? "animate-reveal-up" : "opacity-0"}`}>
          <p className="font-body text-sm tracking-[0.3em] uppercase text-primary mb-4">{t("concerts.label")}</p>
        ...
        <div className="divide-y divide-border">
          {concerts.map((concert, i) => (
            <div
              key={i}
              className={`group flex flex-col gap-2 py-7 transition-colors hover:bg-secondary/30 sm:flex-row sm:items-center sm:gap-0 sm:px-4 cursor-pointer ${
                isVisible ? "animate-reveal-up" : "opacity-0"
              }`}
              style={isVisible ? { animationDelay: `${150 + i * 80}ms` } : undefined}
            >
              <span className="font-body w-40 text-sm tracking-[0.15em] uppercase text-primary shrink-0">{concert.date}</span>
              <span className="font-display text-2xl font-light md:text-3xl flex-1">{concert.city}</span>
              <span className="font-body text-base text-foreground/75">{concert.venue}</span>
